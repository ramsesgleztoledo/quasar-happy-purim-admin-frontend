...
import { ref } from 'vue';
import type { AuxFormField, FormField } from './interfaces';
...
interface FormComposition {
  ...
  onFormReset: (newForm?: { [key: string]: any }, original?: boolean) => void;
  ...
  isValidField: (fieldLocation: (string | number)[], withErrors?: boolean) => { valid: boolean, [key: string]: any } | boolean
  getFormValue: () => {
    [x: string]: any;
  }
}

export default (form: { [key: string]: AuxFormField }): FormComposition => {
  /**============================================
 ...
 *=============================================**/
  ...
  const prepareForm = () => {
    ...
    const auxForm: { [key: string]: FormField | any[] } = {};
...
//* fill the array with the rest of the objects or arrays
  ...
  const realForm = ref({ ...prepareForm() })
 ...
 *=======================================================================================================================**/
  ...
  const onFieldChange = (
    ...
    const auxForm = { ...realForm.value };

    if (fieldLocation.length === 1 && itIsAProp(fieldLocation[0], auxForm)) {
      const auxValue = auxForm[fieldLocation[0]]
      if (auxValue && !('value' in auxValue))
        throw new Error(
  ...
  const itIsAProp = (prop: string | number, defaultForm: {
    [key: string]: any
  }) => {
  ...
  const findTheField = (auxForm: {
    ...
    for (let i = 0; i < fieldLocation.length; i++) {
      const auxValue = fieldLocation[i]

      if (auxValue) {
        checkTheProp(element, auxValue);
        element = element[auxValue];
      }
    }
 ...
 *========================================================================**/
  ...
  const resetForm = (original: boolean) => {
 ...
 *========================================================================**/
  const getValue = (fieldLocation: (number | string)[]) => {
